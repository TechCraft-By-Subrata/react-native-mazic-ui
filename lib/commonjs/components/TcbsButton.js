...
const TcbsButton = ({ title, onPress, size = TcbsButton_types_1.BUTTON_SIZE.LARGE, variant = TcbsButton_types_1.BUTTON_VARIANT.PRIMARY, borderRadius, disabled = false, style, textStyle, iconName, iconGroup = 'MaterialIcons', iconColor, iconSize, iconPosition = 'top', accessibilityLabel, accessibilityHint, accessibilityRole = 'button', accessibilityState, }) => {
    ...
    const { themeColors, tcbsTheme, scaleTokens } = (0, themeStore_1.useTcbsColorStore)();
    ...
    const buttonStyle = (0, react_1.useMemo)(() => {
        ...
        if (borderRadius === TcbsButton_types_1.BORDER_RADIUS.NONE) {
            ...
            computedBorderRadius =
                size === TcbsButton_types_1.BUTTON_SIZE.LARGE
                    ? scaleTokens.radius.m
                    : size === TcbsButton_types_1.BUTTON_SIZE.MEDIUM
    ...
    const themedTextStyle = (0, react_1.useMemo)(() => {
        return Object.assign({ color: resolvedTextColor, fontSize: size === TcbsButton_types_1.BUTTON_SIZE.LARGE
                ? scaleTokens.fontSize.xl
                : size === TcbsButton_types_1.BUTTON_SIZE.MEDIUM
                    ? scaleTokens.fontSize.m
                    : scaleTokens.fontSize.s, fontWeight: '700' }, (react_native_1.StyleSheet.flatten(textStyle) || {}));
    }, [size, resolvedTextColor, textStyle, scaleTokens]);
    const renderIcon = (IconComponent) => (react_1.default.createElement(IconComponent, { name: iconName, size: iconSize ||
            (size === TcbsButton_types_1.BUTTON_SIZE.LARGE
                ? scaleTokens.fontSize.xxl + scaleTokens.spacing.m
                : size === TcbsButton_types_1.BUTTON_SIZE.MEDIUM
                    ? scaleTokens.fontSize.xl + scaleTokens.spacing.s
                    : scaleTokens.fontSize.l + scaleTokens.spacing.s), color: iconColor || resolvedTextColor, style: iconPosition === 'top'
    ...
    const renderText = (customStyle) => {
        ...
        const finalStyle = customStyle
            ? Object.assign({ color: resolvedTextColor, fontSize: size === TcbsButton_types_1.BUTTON_SIZE.LARGE
                    ? scaleTokens.fontSize.l
                    : size === TcbsButton_types_1.BUTTON_SIZE.MEDIUM
                        ? scaleTokens.fontSize.s
                        : scaleTokens.fontSize.xs, fontWeight: '500' }, customStyle) : themedTextStyle;
