...
export const TcbsButton = ({ title, onPress, size = BUTTON_SIZE.LARGE, variant = BUTTON_VARIANT.PRIMARY, borderRadius, disabled = false, style, textStyle, iconName, iconGroup = 'MaterialIcons', iconColor, iconSize, iconPosition = 'top', accessibilityLabel, accessibilityHint, accessibilityRole = 'button', accessibilityState, }) => {
    ...
    const themedTextStyle = useMemo(() => {
        return Object.assign({ color: resolvedTextColor, fontSize: size === BUTTON_SIZE.LARGE
                ? scaleTokens.fontSize.xl
                : size === BUTTON_SIZE.MEDIUM
                    ? scaleTokens.fontSize.m
                    : scaleTokens.fontSize.s, fontWeight: '700' }, (StyleSheet.flatten(textStyle) || {}));
    }, [size, resolvedTextColor, textStyle, scaleTokens]);
    const renderIcon = (IconComponent) => (React.createElement(IconComponent, { name: iconName, size: iconSize ||
            (size === BUTTON_SIZE.LARGE
    ...
    const renderText = (customStyle) => {
        ...
        const finalStyle = customStyle
            ? Object.assign({ color: resolvedTextColor, fontSize: size === BUTTON_SIZE.LARGE
